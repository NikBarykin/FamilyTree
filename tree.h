...
namespace FamilyTree {
    template<class NodeId, size_t NParents, class NodeIdHash = std::hash<NodeId>, class NodeIdEqual = std::equal_to<NodeId>>
    class Tree {
    public:
        using Node = Node<NodeId, NParents>;
        template<class ValT>
        using NodeIdMap = std::unordered_map<NodeId, ValT, NodeIdHash, NodeIdEqual>;
        using NodeIdSet = std::unordered_set<NodeId, NodeIdHash, NodeIdEqual>;
    private:
        NodeIdMap<Node> nodes_;
    ...
    public:
        ...
        static Tree ParseFrom(const std::string& input)  {
            ...
            return Tree(nodes.begin(), nodes.end());
        }
        ...
        NodeIdSet GetAncestors(const NodeId &node) const {
            NodeIdSet ancestors;
            std::queue<NodeId> node_order;
            node_order.push(node);
            NodeIdSet considered_nodes = {node};
        ...
        NodeIdSet LowestCommonAncestors(const NodeId &node1, const NodeId &node2) const {
    ...
    private:
        ...
        NodeIdMap<Svg::Color> CalculateColors() const {
            NodeIdMap<Svg::Color> colors;
        ...
        std::vector<std::vector<NodeId>> DistributeNodesInLevels() const {
            ...
            NodeIdMap<size_t> level_by_node;
            auto nodes = GetNodes();
        ...
        NodeIdMap<Svg::Point> CalculatePositions() const {
            auto levels = DistributeNodesInLevels();
            NodeIdMap<Svg::Point> positions;
    ...
    std::ostream& operator <<(std::ostream& output,
                              const Tree<NodeId, NParents, NodeIdHash, NodeIdEqual>& tree) {
